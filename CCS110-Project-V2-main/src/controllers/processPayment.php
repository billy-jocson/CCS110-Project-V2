...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if ($id) {
        ...
        if ($paymentData) {
            ...
            $mysql3 = "INSERT INTO salary_history (employee_id, salary_id, salary_amount, bonus, deduction) VALUES (?, ?, ?, ?, ?)";
            $stmt3 = $conn->prepare($mysql3);
            $bonus = $paymentData['bonus'];
            $deduction = $paymentData['deduction'];
            $stmt3->bind_param("iidii", $id, $salary_id, $amount, $bonus, $deduction);
            $stmt3->execute();
