...
if ($isAdmin == 0) {
    $sql = "SELECT sh.transaction_id, sh.employee_id, sh.date, sh.salary_amount, sh.bonus, sh.deduction,
...
if ($query->num_rows == 0) {
    ...
    while ($row = $query->fetch_assoc()) {
        ...
        echo '
            ...
            <td class="px-6 py-4 text-green-600 font-semibold">+₱' . number_format($row["bonus"] ?? 0, 2) . '</td>
            <td class="px-6 py-4 text-red-500 font-semibold">-₱' . number_format($row["deduction"] ?? 0, 2) . '</td>
            <td class="px-6 py-4 text-blue-600 font-semibold">₱' . number_format($row["salary_amount"], 2) . '</td>
        </tr>';
    }
